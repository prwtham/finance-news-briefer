...
if st.session_state.active_view == "Forecasting":
    ...
    if is_company:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f'<div style="padding:24px;background:#FFFFFF;border:1px solid #E0D5C7;border-radius:12px;"><p class="metric-label" style="margin-bottom:16px;">Upside Catalyst</p><div style="width:100%;height:6px;border-radius:3px;background:linear-gradient(to right,#419577,#F5D649);margin-bottom:16px;"></div><p style="font-size:13px;color:#5a5a5a!important;line-height:1.6;">{colorize_numbers(catalyst_text)}</p></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div style="padding:24px;background:#FFFFFF;border:1px solid #E0D5C7;border-radius:12px;"><p class="metric-label" style="margin-bottom:16px;">Risk Exposure</p><div style="width:100%;height:6px;border-radius:3px;background:linear-gradient(to right,#ec4242,#F5AB41);margin-bottom:16px;"></div><p style="font-size:13px;color:#5a5a5a!important;line-height:1.6;">{colorize_numbers(risk_text)}</p></div>', unsafe_allow_html=True)
